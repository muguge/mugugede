<template>
	<div class="diy_edit page_adoption_information" id="adoption_information_edit">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					...
					<div v-if="$check_field('set','name_of_adopter') || $check_field('add','name_of_adopter') || $check_field('get','name_of_adopter')" class="col-12 col-md-6">
						<div class="diy_title">
							<span>领养人姓名:
							</span>
						</div>
						...
						<div class="diy_field diy_text">
							<input type="text" id="form_name_of_adopter" v-model="form['name_of_adopter']" placeholder="请输入领养人姓名" v-if="(form['name_of_adopter'] && $check_field('set','name_of_adopter')) || (!form['name_of_adopter'] && $check_field('add','name_of_adopter'))"  :disabled="disabledObj['name_of_adopter_isDisabled']"/>
							<span v-else-if="$check_field('get','name_of_adopter')">{{ form['name_of_adopter'] }}</span>
						</div>
					</div>
					<div v-if="$check_field('set','contact_information') || $check_field('add','contact_information') || $check_field('get','contact_information')" class="col-12 col-md-6">
						...
						<div class="diy_field diy_text">
							...
							<span v-else-if="$check_field('get','contact_information')">{{ form['name_of_adopter'] }}</span>
						</div>
					</div>
					<div v-if="$check_field('set','animal_name') || $check_field('add','animal_name') || $check_field('get','animal_name')" class="col-12 col-md-6">
						<div class="diy_title">
							<span>动物名称:
							</span>
						</div>
						<!-- 文本 -->
						<div class="diy_field diy_text">
							<input type="text" id="form_animal_name" v-model="form['animal_name']" placeholder="请输入动物名称" v-if="(form['animal_name'] && $check_field('set','animal_name')) || (!form['animal_name'] && $check_field('add','animal_name'))"  :disabled="disabledObj['animal_name_isDisabled']"/>
							<span v-else-if="$check_field('get','animal_name')">{{ form['name_of_adopter'] }}</span>
						</div>
					</div>
					<div v-if="$check_field('set','adoption_notes') || $check_field('add','adoption_notes') || $check_field('get','adoption_notes')" class="col-12 col-md-6">
						<div class="diy_title">
							<span>领养备注:
							</span>
						</div>
						...
						<div class="diy_field diy_desc">
							<textarea id="form_adoption_notes" v-model="form['adoption_notes']" v-if="(form['adoption_notes'] && $check_field('set','adoption_notes')) || (!form['adoption_notes'] && $check_field('add','adoption_notes'))" :disabled="disabledObj['adoption_notes_isDisabled']" />
							<span v-else-if="$check_field('get','adoption_notes')">{{ form['name_of_adopter'] }}</span>
						</div>
					</div>
					<div v-if="user_group === '管理员' || $check_examine()" class="col-12 col-md-6">
						...
						<div class="diy_field diy_select" v-if="$check_action('/adoption_information/edit','examine')">
					...
					<div v-if="user_group === '管理员' || $check_examine()" class="col-12 col-md-6" >
						...
						<div class="diy_field diy_desc" v-if="$check_action('/adoption_information/edit','examine')">
	...
	export default {
		...
		methods: {
			...
			change_file(files, str) {
				...
				this.$post("~/api/adoption_information/upload?", form, (res) => {
					if (res.result) {
