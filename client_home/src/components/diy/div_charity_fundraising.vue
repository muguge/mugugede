<template>
	<div class="diy_details_box diy_div_charity_fundraising">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "charity_fundraising_id",
					source_id: this.obj["charity_fundraising_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/charity_fundraising/set?charity_fundraising_id=' + _this.obj["charity_fundraising_id"], {
								praise_len
							}, (res) => {
								if(res.result){
