<template>
	<div class="diy_details_box diy_div_volunteer_users">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "volunteer_users_id",
					source_id: this.obj["volunteer_users_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/volunteer_users/set?volunteer_users_id=' + _this.obj["volunteer_users_id"], {
								praise_len
							}, (res) => {
								if(res.result){
