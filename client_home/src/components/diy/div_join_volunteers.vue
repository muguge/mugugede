<template>
	<div class="diy_details_box diy_div_join_volunteers">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "join_volunteers_id",
					source_id: this.obj["join_volunteers_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/join_volunteers/set?join_volunteers_id=' + _this.obj["join_volunteers_id"], {
								praise_len
							}, (res) => {
								if(res.result){
