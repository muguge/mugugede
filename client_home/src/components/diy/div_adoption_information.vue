		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "adoption_information_id",
					source_id: this.obj["adoption_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/adoption_information/set?adoption_information_id=' + _this.obj["adoption_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
