<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','volunteer_user_name') || $check_field('add','volunteer_user_name') || $check_field('set','volunteer_user_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="志愿者用户名" prop="volunteer_user_name">
					<el-input id="volunteer_user_name" v-model="form_sub['volunteer_user_name']" placeholder="请输入志愿者用户名"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','volunteer_user_name')) || (!form_sub['volunteer_users_id'] && $check_field('add','volunteer_user_name'))"></el-input>
					<div v-else-if="$check_field('get','volunteer_user_name')">{{form_sub['volunteer_user_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','volunteer_number') || $check_field('add','volunteer_number') || $check_field('set','volunteer_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="志愿者编号" prop="volunteer_number">
					<el-input id="volunteer_number" v-model="form_sub['volunteer_number']" placeholder="请输入志愿者编号"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','volunteer_number')) || (!form_sub['volunteer_users_id'] && $check_field('add','volunteer_number'))" :disabled="true"></el-input>
					<div v-else-if="$check_field('get','volunteer_number')">{{form_sub['volunteer_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','full_name')) || (!form_sub['volunteer_users_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','gender')) || (!form_sub['volunteer_users_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','age') || $check_field('add','age') || $check_field('set','age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="年龄" prop="age">
					<el-input id="age" v-model="form_sub['age']" placeholder="请输入年龄"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','age')) || (!form_sub['volunteer_users_id'] && $check_field('add','age'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','id') || $check_field('add','id') || $check_field('set','id')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="身份证" prop="id">
					<el-input id="id" v-model="form_sub['id']" placeholder="请输入身份证"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','id')) || (!form_sub['volunteer_users_id'] && $check_field('add','id'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','address') || $check_field('add','address') || $check_field('set','address')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="地址" prop="address">
					<el-input id="address" v-model="form_sub['address']" placeholder="请输入地址"
							  v-if="user_group === '管理员' || (form_sub['volunteer_users_id'] && $check_field('set','address')) || (!form_sub['volunteer_users_id'] && $check_field('add','address'))"></el-input>
					<div v-else-if="$check_field('get','address')">{{form_sub['address']}}</div>
				</el-form-item>
			</el-col>
			<el-col :xs="24" :sm="12" :lg="8">
				<el-form-item>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					"volunteer_user_name_isDisabled": false,
					"volunteer_number_isDisabled": false,
				...
				form_sub: {
					"volunteer_user_name":'', // 志愿者用户名
					"volunteer_number":this.$get_stamp(), // 志愿者编号
					"full_name":'', // 姓名
					"gender":'', // 性别
					"age":'', // 年龄
					"id":'', // 身份证
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["volunteer_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?volunteer_users_id=" + form_sub["volunteer_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/volunteer_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer_users/view','get');
