<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','user_number') || $check_field('add','user_number') || $check_field('set','user_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="用户编号" prop="user_number">
					<el-input id="user_number" v-model="form_sub['user_number']" placeholder="请输入用户编号"
							  v-if="user_group === '管理员' || (form_sub['ordinary_users_id'] && $check_field('set','user_number')) || (!form_sub['ordinary_users_id'] && $check_field('add','user_number'))" :disabled="true"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','address') || $check_field('add','address') || $check_field('set','address')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="地址" prop="address">
					<el-input id="address" v-model="form_sub['address']" placeholder="请输入地址"
							  v-if="user_group === '管理员' || (form_sub['ordinary_users_id'] && $check_field('set','address')) || (!form_sub['ordinary_users_id'] && $check_field('add','address'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				form_sub: {
					"user_name":'', // 用户名
					"user_number":this.$get_stamp(), // 用户编号
					"full_name":'', // 姓名
					"gender":'', // 性别
					"age":'', // 年龄
					"id":'', // 身份证
