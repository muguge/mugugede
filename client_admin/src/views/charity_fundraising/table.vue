	...
	<el-main class="bg">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
				...
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="募捐类型">
						<el-input v-model="query.fundraising_type"></el-input>
					</el-form-item>
				</el-col>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/charity_fundraising/table','add') || $check_action('/charity_fundraising/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/charity_fundraising/table','del') || $check_action('/charity_fundraising/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="fundraising_type" label="募捐类型"
				v-if="user_group == '管理员' || $check_field('get','fundraising_type')" min-width="200">
			</el-table-column>
			<el-table-column prop="item_name" label="物品名称"
				v-if="user_group == '管理员' || $check_field('get','item_name')" min-width="200">
			</el-table-column>
			<el-table-column prop="amount_of_contributions" label="捐款数额"
				v-if="user_group == '管理员' || $check_field('get','amount_of_contributions')" min-width="200">
			</el-table-column>
			...
			<el-table-column prop="item_details" label="物品详情"
				v-if="user_group == '管理员' || $check_field('get','item_details')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/charity_fundraising/table','set') || $check_action('/charity_fundraising/view','set') || $check_action('/charity_fundraising/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/charity_fundraising/table','set') || $check_action('/charity_fundraising/view','set') || $check_action('/charity_fundraising/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/charity_fundraising/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/charity_fundraising/get_list?like=0",
				url_del: "~/api/charity_fundraising/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.charity_fundraising_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/charity_fundraising/set?charity_fundraising_id="+this.pay_obj.id);
